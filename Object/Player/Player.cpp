...
void Player::Initialize(Model* model,Model*bulletModel, Vector3 positon) {
	// NULLポインタチェック
	assert(model);
	assert(bulletModel);
...
void Player::Update(const ViewProjection viewProjection_) {
	...
	if (Input::GetInstance()->GetJoystickState(0, joyState))
	{
		move.x += (float)joyState.Gamepad.sThumbLX / SHRT_MAX * kCharcterSpeed;
		move.y += (float)joyState.Gamepad.sThumbLY / SHRT_MAX * kCharcterSpeed;
	}
...
void Player::Draw(ViewProjection viewProjection_) {
	...
	bulletModel_->Draw(worldTransform3DReticle_, viewProjection_);
